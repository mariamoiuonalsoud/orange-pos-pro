...
Orange POS Pro is a modern, high-performance Point of Sale (POS) solution tailored for retail businesses. Developed using **React 18**, **TypeScript**, and **Supabase**, it provides a robust architecture for handling high-frequency transactions, inventory precision, and hardware integration.
...
- **Thermal Printing**: Custom-tailored CSS for 80mm Receipt Printers (Aiyin/Xprinter), ensuring pixel-perfect branded output.
...
- **End-to-End Testing**: Fully covered by _Playwright_ scripts simulating complete sale journeys (Login -> Scan -> Checkout).
- **Data Integrity**: Strict architectural constraints to ensure financial accuracy and prevent data regression.
- **Robust Selectors**: Implementation of _data-testid_ strategy for stable, maintainable, and reliable test execution.
...
- **Run E2E Tests (Headless)**: npx playwright test
- **Run Tests with UI (Interactive)**: npx playwright test --ui
...
├── hooks/           # Custom Logic (useSupabase, usePrint)
├── tests/           # Playwright E2E Sale Journey Scenarios
